...
with open("t.txt", 'w+') as f:
    for i in l:
        f.write(i.replace('PUSH', 'push').replace('ADD','add').replace('SUB', 'sub').replace('IMUL', 'imul').replace('IDIV','idiv').replace('CDQ','cdq').replace('POP','pop').replace('EAX','eax').replace('EBX','ebx'))