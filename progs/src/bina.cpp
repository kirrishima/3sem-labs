...
void printBits(const vector<bool> &bits)
{
    ...
    {
        if (i % 8 == 0 && i != 0)
            cout << ' '; // Separate bytes for readability
        cout << bits[i];
    }
    cout << '\n';
}
    ...
    else if (mode == "fb" || mode == "from-binary")
    {
        ...
        for (char c : valueStr)
        {
            if (c == '1')
                ...
                throw invalid_argument("Binary number can only contain '0' and '1'.");
        }
        ...
        if (isSigned)
        {
            if (bits.size() == 8)
            {
                char result = fromBinary<char>(bits);
                cout << "Decimal representation (signed, 1 byte): " << (int)result << '\n';
            }
            else if (bits.size() == 16)
            {
                short result = fromBinary<short>(bits);
                cout << "Decimal representation (signed, 2 bytes): " << result << '\n';
            }
            else if (bits.size() == 32)
            {
                int result = fromBinary<int>(bits);
                cout << "Decimal representation (signed, 4 bytes): " << result << '\n';
            }
            else
            {
                long long result = fromBinary<long long>(bits);
                cout << "Decimal representation (signed, 8 bytes): " << result << '\n';
            }
        }
        else
        {
            ull result = fromBinary<ull>(bits);
            cout << "Decimal representation (unsigned, 8 bytes): " << result << '\n';
        }
    }
    else
    {
